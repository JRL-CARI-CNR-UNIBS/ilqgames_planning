...
TEST(Polyline3Test, ClosestPointWorks) {
  const Point3 p1(0.0, -1.0);
  const Point3 p2(0.0, 1.0);
  const Point3 p3(2.0, 1.0);
  ...
  EXPECT_NEAR(signed_squared_distance, 2.0, ilqgames::constants::kSmallNumber);

  query << 0.5, 0.0;
  closest = polyline.ClosestPoint(query, &is_vertex, &segment,
                                  &signed_squared_distance);
  EXPECT_FALSE(is_vertex);
  EXPECT_LT(closest.squaredNorm(), ilqgames::constants::kSmallNumber);
  EXPECT_NEAR(signed_squared_distance, 0.5 * 0.5, ilqgames::constants::kSmallNumber);

  query << 1.5, 0.0;
  closest = polyline.ClosestPoint(query, &is_vertex, &segment,
                                  &signed_squared_distance);
  EXPECT_FALSE(is_vertex);
  EXPECT_TRUE(closest.isApprox(Point3(1.5, 1.0)));
  EXPECT_NEAR(signed_squared_distance, 1.0, ilqgames::constants::kSmallNumber);
  ...
  closest = polyline.ClosestPoint(query, &is_vertex, &segment,
                                  &signed_squared_distance);
  EXPECT_FALSE(is_vertex);
  EXPECT_TRUE(closest.isApprox(Point3(0.5, 1.0)));
  EXPECT_NEAR(signed_squared_distance, -1.0, ilqgames::constants::kSmallNumber);
